...
import numpy as np
from models.utils import AverageMeter, get_scheduler, psnr, get_nonlinearity, get_norm
...
''' Adapative DenseNet '''
class _DenseLayer_Last(nn.Module):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate, memory_efficient=False):
        super(_DenseLayer_Last, self).__init__()
        self.add_module('norm1', nn.BatchNorm2d(num_input_features)),
        self.add_module('relu1', nn.ReLU(inplace=True)),
        self.add_module('conv1', nn.Conv2d(num_input_features, bn_size *
                                           growth_rate, kernel_size=1, stride=1,
...
class _DenseBlock_Last(nn.ModuleDict):
    _version = 2

    def __init__(self, num_layers, num_input_features, bn_size, growth_rate, drop_rate, memory_efficient=False):
        super(_DenseBlock_Last, self).__init__()
        for i in range(num_layers):
            layer = _DenseLayer_Last(
                num_input_features + i * growth_rate,
                growth_rate=growth_rate,
                bn_size=bn_size,
                drop_rate=drop_rate,
...
class _Transition_Last(nn.Sequential):
    def __init__(self, num_input_features, num_output_features):
        super(_Transition_Last, self).__init__()
        self.add_module('norm', nn.BatchNorm2d(num_input_features))
        self.add_module('relu', nn.ReLU(inplace=True))
        self.add_module('conv', nn.Conv2d(num_input_features, num_output_features,
                                          kernel_size=1, stride=1, bias=False))
...
class DenseNetADA(nn.Module):
    def __init__(self, in_channels=1, growth_rate=32, block_config=(6, 12, 24, 16),
                 num_init_features=64, bn_size=4, drop_rate=0, num_classes=14, memory_efficient=False,
                 num_maps=3, kmax=1, kmin=None, alpha=1):

        super(DenseNetADA, self).__init__()
        ...
        for i, num_layers in enumerate(block_config):
            ...
            if i != len(block_config) - 1:
                if i < len(block_config) - 2:
                    ...
                    trans = _Transition_Last(num_input_features=num_features,
                                             num_output_features=num_features // 2)
                self.features.add_module('transition%d' % (i + 1), trans)
                num_features = num_features // 2
